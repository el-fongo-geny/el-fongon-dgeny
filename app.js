...
function orderFromBackend(row) {
  ...
  /*
    V104.8A.3
    admin-orders devuelve las columnas reales de Supabase.
    El panel histórico trabaja con customer{} y totals{}.
    Normalizamos ambos formatos aquí para no perder información.
  */
  const rawCustomer = row.customer || row.raw?.customer || {};
  const customer = {
    ...rawCustomer,
    name:
      row.customer_name ??
  ...
  const rawTotals = row.totals || row.raw?.totals || {};
  ...
  return {
    ...
    orderType:
      ...
      row.raw?.orderType ||
      (Array.isArray(row.items) && row.items[0]?.orderType) ||
      "",
