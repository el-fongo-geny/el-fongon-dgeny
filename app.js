...
function isProductVisible(item) {
  if (!item) return false;
  return availabilityValue(productAvailabilityKey(item), item.id);
}

function isProductAvailable(item) {
  if (!isProductVisible(item)) return false;
...
function setLanguage(lang) {
  ...
  if (languageGate) languageGate.classList.add("hidden");
...
function setupCategoryObserver() {
  ...
  categoryObserver = new IntersectionObserver((entries) => {
    ...
    if (nextCategory && nextCategory !== state.category) {
      ...
      const active = Array.from(document.querySelectorAll("[data-category]"))
    ...
    <div class="modal-sheet">
      ...
      <form id="productForm" class="product-form">
        ${groups.map((group) => `
          <fieldset>
            <legend>
              ${group[state.lang] || group.es}
              <span>${group.required ? text("required") : text("optional")}</span>
            </legend>
        ...
        ${removables.length ? `
          <fieldset>
            <legend>
              ${state.lang === "en" ? "Remove ingredients" : "Quitar ingredientes"}
        ...
        <button class="primary-btn full sticky-add" type="submit">
          ${text("addToCart")} · ${money(item.price)}
        </button>
      </form>
    </div>
  `;
...
function buildCartItem(form) {
  ...
  Array.from(form.querySelectorAll(`[name="removables"]:checked`)).forEach((input) => {
    ...
    if (remove) {
      removables.push(remove[state.lang] || remove.es);
    }
  });
...
function getTotals() {
  const subtotal = state.cart.reduce((sum, item) => {
    return sum + item.lineTotal * item.quantity;
  }, 0);

  const tax = state.cart.reduce((sum, item) => {
    if (!item.taxable) return sum;
    return sum + item.lineTotal * item.quantity * item.taxRate;
  }, 0);
...
async function saveOrder(paymentMethod) {
  ...
  if (db?.isReady()) {
    ...
    try {
      ...
      if (result?.orderId) {
        order.backendOrderId = result.orderId;
      }
    } catch (error) {
...
function initEvents() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest("#themeToggleBtn")) {
      toggleTheme();
      return;
    }
...
function init() {
  ...
  window.addEventListener("storage", (event) => {
    if (event.key === "fogon_availability" || event.key === null) {
      refreshAvailabilityIfChanged(true);
    }
  });
