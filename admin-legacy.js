...
function itemHtml(item){var h='<div class="item"><strong>'+esc(item.quantity||1)+'x '+esc(item.name||"")+'</strong>';var a=item.selections||[],i;for(i=0;i<a.length;i++)h+='<p>'+esc(a[i].group)+': '+esc(a[i].name)+'</p>';a=item.extras||[];for(i=0;i<a.length;i++)h+='<p>Extra: '+esc(a[i].name)+'</p>';if(item.notes)h+='<p><strong>Nota:</strong> '+esc(item.notes)+'</p>';return h+'</div>';}
